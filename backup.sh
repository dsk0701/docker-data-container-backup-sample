...
# バックアップを行います。
docker run --rm --volumes-from ${STORAGE_CONTAINER_NAME} -v ${HOST_BACKUP_DIR}:${CONTAINER_BACKUP_DIR} ubuntu tar zcvf ${CONTAINER_BACKUP_DIR}/backup-${BACKUP_FILE_NAME}.tar.gz ${CONTAINER_BACKUP_DATA_DIR}
